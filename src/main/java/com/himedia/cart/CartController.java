...
import com.himedia.cartitem.CartItemRepository;
import com.himedia.item.ItemImgRepository;
import com.himedia.item.ItemSellingInformRepository;
...
public class CartController {
	...
	public String openCartList(Model model,Principal principal) {
		...
		for(int i=0 ; i < cartItemList.size() ; i++) {
			...
			ItemImg itemImg = this.itemImgRepository.findAllByItemAndRepImgYn(cartItemList.get(i).getItemSellingInform().getItem().getId(),"Y");
